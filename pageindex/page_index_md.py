...
async def get_node_summary(node, summary_token_threshold=200, model=None):
    ...
    # Simplified: use character count instead of token count
    if len(node_text) < summary_token_threshold * 4:  # Approximate 4 chars per token
        return node_text
    else:
...
def update_node_list_with_text_token_count(node_list, model=None):
    ...
    for i in range(len(result_list) - 1, -1, -1):
        ...
        # Simplified: use character count instead of token count
        result_list[i]['text_token_count'] = len(total_text)
...
def tree_thinning_for_index(node_list, min_node_token=None, model=None):
    ...
    for i in range(len(result_list) - 1, -1, -1):
        ...
        total_chars = current_node.get('text_token_count', 0)  # Now represents character count

        if total_chars < min_node_token * 4:  # Approximate 4 chars per token
            children_indices = find_all_children(i, current_level, result_list)
            ...
            if children_texts:
                ...
                result_list[i]['text_token_count'] = len(merged_text)  # Character count
